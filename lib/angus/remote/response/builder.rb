    ...
    module Response
      module Builder
        ...
        def self.build(status_code, body, service_code_name, service_version, operation_namespace,
          ...
          build_from_definition(status_code, body, service_code_name, service_version,
        ...
        def self.build_from_definition(status_code, body, service_code_name, service_version,
          ...
          fields[:messages] = build_messages(json_response['messages'])
          fields[:body] = body
          ...
          operation_definition.response_elements.each do |element|
            element_value = build_response_method(json_response, representations_hash, glossary_terms_hash,
                                                  element)
        ...
        def self.build_response_method(json_response, representations_hash, glossary_terms_hash, element)
          ...
          if element.type && representations_hash.include?(element.type)
            object_value = build_from_representation(hash_value, element.type,
                                                     representations_hash, glossary_terms_hash)
          elsif element.elements_type
            object_value = build_collection_from_representation(hash_value,
                                                                element.elements_type,
                                                                representations_hash,
                                                                glossary_terms_hash)
          elsif element.type && element.type.to_sym == :variable
            object_value = build_from_variable_fields(hash_value)
          elsif element.type
            begin
        ...
        def self.build_from_remote_response(remote_response, service_code_name, version,
          ...
          build(status_code, body, service_code_name, version, operation_namespace,
                operation_code_name)
        end
        ...
        def self.apply_glossary(name, glossary_terms_hash)
          name = glossary_terms_hash[name].long_name if glossary_terms_hash.include?(name)
            ...
            representation.fields.each do |field|
              ...
              unless field_raw_value.nil? && field.required == false
                if field.type && representations.include?(field.type)
                  field_value = build_from_representation(field_raw_value, field.type,
                                                          representations,
                                                          glossary_terms_hash)
                elsif field.elements_type
                  field_value = build_collection_from_representation(field_raw_value,
                                                                     field.elements_type,
                                                                     representations,
                                                                     glossary_terms_hash)
                elsif field.type && field.type.to_sym == :variable
                  field_value = build_from_variable_fields(field_raw_value)
                elsif field.type
